...
public class BuscadorDeLaBibl {
    ...
    public ListaConPI<Termino> hapax() {
        ...
        ListaConPI<Termino> claus = index.claves();
        claus.inicio(); res.inicio();
        for (int i = 0; i < claus.talla(); i++) {
            ListaConPI<BuscadorDeLaBibl.Posting> valor = index.recuperar(claus.recuperar());
            if (valor != null && valor.talla() == 1) { res.insertar(claus.recuperar()); }
            claus.siguiente();
        }
        if (res.talla() > 0) return res;
        return null;
    }
}    
