...
public class ApuestaPrimitiva {
    ...
    protected int posicionDe(NumeroPrimitiva n) {
        ...
        combinacion.inicio();
        int posicion = 0;
        while (!combinacion.esFin() && !trobat) {
            NumeroPrimitiva aux = combinacion.recuperar();
            if (aux.equals(n)) { trobat = true; }
            else {
                combinacion.siguiente();
                posicion++;
            }
        }
        if (trobat) return posicion;
        return -1;
    }
