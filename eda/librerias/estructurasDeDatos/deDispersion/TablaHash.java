...
public class TablaHash<C, V> implements Map<C, V> {
    ...
    // metodo main a incluir en la clase TablaHash para el test del metodo eliminarValoresDuplicados
    public static void main(String[] args) {
         TablaHash<Integer,Integer> tabla = new TablaHash(30);
         for(int i=0;i<10;i++){
             tabla.insertar(i,i);
             tabla.insertar(i+10,5);
             tabla.insertar(i+20,6);
         }
         System.out.println("Talla antes de eliminar: "+tabla.talla());
         tabla.eliminarValoresDuplicados(tabla);
         System.out.println("Talla despuÃ©s de eliminar: "+tabla.talla());
        
    }
    
    public static void eliminarValoresDuplicados(Map<Integer,Integer> tabla){
        Map<Integer, Integer> res = new TablaHash<Integer, Integer>(tabla.talla());
        ListaConPI <Integer> claves = tabla.claves();
        for (claves.inicio(); !claves.esFin(); claves.siguiente()){
            Integer clave = claves.recuperar();
            Integer valor = tabla.recuperar(clave);
            Integer cont = tabla.recuperar(valor); 
            
            //si hay dos iguales, me he quedado sin tiempo, pero 
            //hay que meter el nuevo valor a un array y recorrerlo
            //para comprobar que solo hay uno repetido y si hay más eliminarlo
            if(cont>1){
    ...
    protected final void rehashing() {
        // COMPLETAR (HECHO)
        int novaCapacitat = siguientePrimo(elArray.length * 2);
        ListaConPI<EntradaHash<C,V>>[] arrayAntic = elArray;
        elArray = new LEGListaConPI[novaCapacitat];
        for (int i = 0; i < elArray.length; i++) {
            elArray[i] = new LEGListaConPI<EntradaHash<C,V>>();
        }
        this.talla = 0;
        for (int i = 0; i < arrayAntic.length; i++) {
            ListaConPI<EntradaHash<C,V>> cubeta = arrayAntic[i];
            cubeta.inicio();
            while (! cubeta.esFin()) {
    ...
    public final double costeMLocalizar() {
        ...
        for (int i = 0; i < elArray.length; i++) {
            ...
            colisiones += longitudCubeta * (longitudCubeta - 1) / 2;
        }
