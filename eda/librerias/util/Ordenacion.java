...
public class Ordenacion {
    ...
    */
    ...
    public static <T extends Comparable<T>> void mergeSort2(T[] v) {
        T[] ordenado = mergeSort2(v, 0, v.length - 1);
    ...
    private static <T extends Comparable<T>> T[] mergeSort2(T[] v,
        ...
        //Solo para que con uno y dos elementos funcione un poco más rápido
        if (f == i) { //queda 1 elemento(f - i == 0), ya está ordenado
            T[] aux = (T[]) new Comparable[1];
            aux[0] = v[i];
            return aux;
        }
        else if (f - i == 1) { //quedan 2 elementos
    ...
    private static <T extends Comparable<T>> T[] merge2(T[] v1, T[] v2) {    
        ...
        while (i < v1.length && j < v2.length) {
         ...
         // Solo ha acabado uno, por tanto ahora metemos el otro que aun no se ha recorrido
